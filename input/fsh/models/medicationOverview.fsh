...
// patient and medicationTreatmentLine are typed as inline logical models (not Reference(...))
// so structure maps can populate their fields directly. Reference(PatientLM) caused
// matchbox to bind the target alias to a Reference (no .name property),
// breaking MedicationOverviewMinToMedicationOverviewLM.
